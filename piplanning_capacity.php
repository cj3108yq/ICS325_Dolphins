<?php
	...
	if (isset($_POST['select-art'])) {
		$selected_art = $_POST['select-art'];

		$sql = "SELECT * FROM `capacity` where team_id='".$selected_art."' AND program_increment='".$program_increment."';";
		$result = $db->query($sql);
		if ($result->num_rows > 0) {
		} else {
			$default_data = true;
			...
			$sql = "SELECT * FROM `membership` where team_name='".$selected_art."';";
			$result = $db->query($sql);
			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
...
<div class="right-content">
		<div class="container">
			...
			<table id="info" width="15%">
				<tr>
					<td width="45%" style="vertical-align: top; font-weight: bold; color: #01B0F1; line-height: 130%; font-size: 18px;">
					...
					<td  style="vertical-align: top; font-weight: bold; line-height: 130%;  font-size: 18px;" width="25%">
						<select name="select-art" onchange="this.form.submit()" style="border: 0; text-align: left;">
							...
							$sql = "SELECT team_id, team_name FROM `trains_and_teams` where type='ART';";
							$result = $db->query($sql);

							if ($result->num_rows > 0) {

									while ($row = $result->fetch_assoc()) {
										if ( trim($selected_art) == trim($row["team_id"]) ) {
			...
			<table id="info2" cellpadding="2px" cellspacing="0" border="0" class="capacity-table"
